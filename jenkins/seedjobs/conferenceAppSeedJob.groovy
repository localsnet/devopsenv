// def gitUrl = 'https://github.com/localsnet/hello-world-servlet'
def gitUrl = 'https://github.com/localsnet/hello-world-servlet'
...
createDockerStopJob("helloworld-app", "app")

createCiJob("helloworld-app-monitoring", gitUrl, "monitoring/pom.xml")
createSonarJob("helloworld-app-monitoring", gitUrl, "monitoring/pom.xml")
createDockerBuildJob("helloworld-app-monitoring", "monitoring")
createDockerStartJob("helloworld-app-monitoring", "monitoring", "58080")
createDockerStopJob("helloworld-app-monitoring", "monitoring")
  ...
  job("${jobName}-4-docker-start-container") {
    ...
    steps {
      steps {
        ...
        shell("sudo /usr/bin/docker run -d --name helloworld-${folder} -p=${port}:8080 helloworld-${folder}")
      }
    }
    publishers {
      chucknorris()
...
def createDockerStopJob(def jobName, def folder) {
  ...
  job("${jobName}-5-docker-stop-container") {
    ...
    steps {
      steps {
        shell("sudo /usr/bin/docker stop \$(sudo /usr/bin/docker ps -a -q --filter=\"name=helloworld-${folder}\")")
        shell("sudo /usr/bin/docker rm \$(sudo /usr/bin/docker ps -a -q --filter=\"name=helloworld-${folder}\")")
      }
    }
    publishers {
      chucknorris()
